...
const TwitterWalletButton: FC<{ web3: ethers.providers.Web3Provider; setWeb3: Dispatch<SetStateAction<ethers.providers.Web3Provider>>; setBiconomy: Dispatch<SetStateAction<Biconomy>>; onClick?: CallableFunction } & ButtonHTMLAttributes<HTMLButtonElement>> = ({ web3, setWeb3, setBiconomy, onClick = () => null, ...props }) => {
	...
	const connectWallet = async () => {
		...
		if (!isLoggedIn) return (magic.oauth as OAuthExtension).loginWithRedirect({ provider: 'twitter', redirectURI: window.location.href })
	...
	useEffect(() => {
		...
		if (uriParams.get('provider') !== 'twitter') return
...
export default TwitterWalletButton
